...
output_file = 'n2_check_highTheta_ac3p5' # Just name, no extension!!
		...
		for j in range(N):
			...
			try:
				...
				for l in range(len(ll_1)):
					...
					# print l, ratio[l]
				ratio_max[j] = max(ratio)
				f.write(str(ratio_max[j]) + '\t\t') # write this value of H0 to the file
				print "ac %f theta_i %f ratio %f"%(ac,Theta_initial[j],max(ratio))
			except CosmoComputationError: # this happens when CLASS fails
				pass # eh, don't do anything
...
print ratio_max
ax1.plot(Theta_initial/np.pi,ratio_max,'r')
ax1.set_xlabel(r"$\Theta_i$", fontsize=16)
ax1.set_ylabel(r"max$\big(\Delta C_\ell(\Theta_i)/C_\ell(\Theta_i=2.5)\big)$", fontsize=16)
ax1.set_title(r"Log10$(a_c)=%.1f$, $n=%.1f$"%(ac,n_axion), fontsize=16)
